...
def json2docx(config, json_list, data, mode="clean"):
    ...
    if os.path.exists("temp/img-TOPOLOGY.png") and os.path.exists("temp/img-TOPOLOGY_cam2.png"):
    ...
    if report_type == 'full':
        ...
        if (os.path.isfile(nomPng)):
            ...
            elif (not os.path.isfile(nomPng2) and os.path.isfile(nomPng)):
    ...
    for i, jsonfile in enumerate(json_list):
        ...
        if 'FREQ' in job_types[i] or ('FREQ' in job_types[i] and 'OPT' in job_types[i]) or ('FREQ' in job_types[i] and 'OPT' in job_types[i] and 'TD' in job_types[i]):
            ...
            t2 = []
            t2.append(["", "", "Table. Most intense (> 50 km/mol) molecular vibrations in wavenumbers"])
            t2.append(["Frequencies", "Intensity", "Symmetry"])
            for k in range(len(vibrational_freq)):
                t2.append(["%d" % vibrational_freq[k], "%d" % vibrational_int[k], vibrational_sym[k]])
            t2.append(["", "", ""])
            table = create_table(doc, t2)
        ...
        if 'TD' in job_types[i] or ('FREQ' in job_types[i] and 'OPT' in job_types[i] and 'TD' in job_types[i]):
            ...
            if rnbExci != 0 and et_energies != 'N/A':
                ...
                if report_type == 'text':
                    ...
                    t.append(["E.S.", "Symmetry", " nm ", "cm⁻¹", "f", "ℝ", "Λ", "d", "q", "Excitation description : initial OM - ending OM (% if > 5%)"])
                ...
                table.columns[0].width = Pt(30);
                table.columns[1].width = Pt(65);
                for column_index in range(2, 9) :
                    table.columns[column_index].width = Pt(35);
                table.columns[9].width = Pt(150);
                table.rows[0].cells[0].merge(table.rows[0].cells[9])
                set_horizontal_line(table, 1, True, 8)
                set_horizontal_line(table, len(t), True, 8)
                run1 = table.cell(1,7).paragraphs[0].add_run("CT")
                run1.italic = True
                run1.font.subscript = True
                run2 = table.cell(1,8).paragraphs[0].add_run("CT")
                run2.italic = True
